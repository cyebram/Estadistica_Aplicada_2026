...
subtitle: "Datos del CPV 2020 - Guanajuato"
format: 
  html:
    grid: 
      body-width: 950px
...
datos_total <- read_csv("data/cpv_valor_11.csv", na = c("ND"))

datos_total <- datos_total |> filter(año == 2020)
#unique(datos_total$indicador)
datos_seleccion <- datos_total |> 
  filter(indicador %in% c("Edad mediana", 
                          "Población total", 
                          "Porcentaje de la población derechohabiente en el IMSS", 
                          "Porcentaje de viviendas con electricidad",
                          "Porcentaje de viviendas particulares habitadas que disponen de Internet",
                          "Porcentaje de población de 12 años y más económicamente activa",
                          "Porcentaje de la población de 15 años y más con instrucción media superior"), desc_municipio != "Estatal") |> 
...
Los nombres de algunas variables (columnas) aún es muy extenso, por lo que es conveniente renombrarlas.
...
datos <- datos |> 
  rename(
    electricidad = porcentaje_de_viviendas_con_electricidad,
    media_superior = porcentaje_de_la_poblacion_de_15_anos_y_mas_con_instruccion_media_superior,
    imss = porcentaje_de_la_poblacion_derechohabiente_en_el_imss,
    pea = porcentaje_de_poblacion_de_12_anos_y_mas_economicamente_activa,
    internet = porcentaje_de_viviendas_particulares_habitadas_que_disponen_de_internet
  )
...
Se unen los datos geograficos de los municipios de Guanajuato con los datos de población y vivienda. Esto nos permitirá crear mapas y realizar análisis espaciales. Para preservar la integridad de los datos, utilizamos `left_join()` de `dplyr`, el primer conjunto de datos debe ser el que contenga la columna `geometry`.

```{r}
#| code-fold: true

datos <- left_join(municipios_gto, datos)
```
...
ggplot(datos) +
  geom_histogram(aes(edad_mediana), color="black", fill = "olivedrab", boundary=min(datos$edad_mediana), binwidth = 1) +
  labs(
...
ggplot(datos) +
  geom_sf(aes(fill = imss), color = "white", size = 0.1) +
  scale_fill_viridis_c(
    name = "IMSS"
  ) +
  labs(
...
ggplot(datos) +
  geom_histogram(aes(imss), color="black", fill = "olivedrab", boundary=min(datos$imss), binwidth = 5) +
  labs(
...
ggplot(datos) +
  geom_density(aes(imss), color="black", fill = "#122ABB", alpha=0.6) +
  labs(
    title = "Derechohabientes IMSS (%)",
...
ggplot(data = datos, aes(x = "", y = imss)) +
    geom_boxplot(
    fill = "#4A90E2",           # Color de relleno
    color = "#2C5F7A",          # Color del borde más oscuro
    alpha = 0.8,                # Transparencia
...
# Q-Q Plot básico para normalidad
ggplot(data = datos, aes(sample = imss)) +
    stat_qq(color = "#4A90E2", size = 2, alpha = 0.7) +
    stat_qq_line(color = "#E74C3C", linewidth = 1, linetype = "dashed") +
    scale_x_continuous(name = "Cuantiles Teóricos (Distribución Normal)",
    breaks = pretty_breaks(n = 6)) +
...
ggplot(datos) +
  geom_sf(aes(fill = pea), color = "white", size = 0.1) +
  scale_fill_viridis_c(
    name = "PEA"
  ) +
  labs(
    title = "PEA (%) de los Municipios de Guanajuato",
    caption = "Fuente: INEGI - CPV 2020"
  ) +
  theme_void() +
  theme(
...
ggplot(datos) +
  geom_histogram(aes(pea), color="black", fill = "olivedrab", boundary=min(datos$pea), binwidth = 2) +
  labs(
    title = "PEA (%)",
    x = "PEA (%)", y = "Frecuencia"
  ) +
...
ggplot(datos) +
  geom_density(aes(pea), color="black", fill = "#122ABB", alpha=0.6) +
  labs(
    title = "PEA (%)",
    x = "PEA (%)", y = "Densidad"
  ) +
...
# Q-Q Plot básico para normalidad
ggplot(data = datos, aes(sample = pea)) +
    stat_qq(color = "#4A90E2", size = 2, alpha = 0.7) +
    stat_qq_line(color = "#E74C3C", linewidth = 1, linetype = "dashed") +
    scale_x_continuous(name = "Cuantiles Teóricos (Distribución Normal)",
    breaks = pretty_breaks(n = 6)) +
    scale_y_continuous(name = "Cuantiles Muestrales (PEA %)",
    breaks = pretty_breaks(n = 6),
    labels = function(x) paste(x, "%")) +
    labs(title = "Gráfico Q-Q para Normalidad",
    subtitle = "PEA (%) - Guanajuato") +
    theme_bw()+
    theme(
    legend.title = element_text(size=13, face="bold"),
    legend.text = element_text(size=13),
...
# Relación entre PEA y Derechohabientes IMSS (%)
...
ggplot(datos) +
    geom_point(aes(x = pea, y = imss), color = "#4A90E2", size = 3, alpha = 0.8) +
    geom_smooth(aes(x = pea, y = imss), method = "lm", color = "#E74C3C", se = TRUE) +
    labs(title = "Relación PEA (%) y Derechohabientes IMSS (%)",
    subtitle = "Estado de Guanajuato",
    x = "PEA (%)", y = "Derechohabientes IMSS (%)") +
    scale_x_continuous(breaks = pretty_breaks(n = 8), labels = function(x) paste(x, "%")) +
    scale_y_continuous(breaks = pretty_breaks(n = 6), labels = function(x) paste(x, "%")) +
    theme_bw()+
    theme(
    legend.title = element_text(size=13, face="bold"),
    legend.text = element_text(size=13),
