...
ggplot(data.frame(x = x_cuant, y = y_cuant), aes(x = x, y = y)) +
  ...
  labs(title = "Cuantiles en la función de distribución",
...
grid.arrange(p1, p2, ncol = 2,
             top = "Ley de los grandes números - distribución exponencial (λ = 2)")
```
...
ggplot(data.frame(x = muestra_grande), aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)), 
                 fill = "lightblue", color = "black", alpha = 0.7, boundary = 0, binwidth=0.1) +
...
ggplot(data.frame(tiempo = tiempos_validos), aes(x = tiempo)) +
  geom_histogram(aes(y = after_stat(density)), 
                 fill = "lightgreen", color = "black", alpha = 0.7, boundary =0, binwidth =0.1) +
