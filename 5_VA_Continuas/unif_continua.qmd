...
cdf_graf <- ggplot(df_cdf) +
  ...
  labs(title = "CDF - Distribución Uniforme",
       x = "x", y = "F(x) = P(X ≤ x)",
       color = "Distribución") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14),
...
ggplot(df_ejemplo, aes(x = x, y = y)) +
  ...
  annotate("text", x = 4, y = 0.05, 
           label = paste0("Área = ", prob_ejemplo), size = 6)+
  theme(
...
```{r simulacion-basica}
# Parámetros de la distribución unif(2, 8)
a <- 2
b <- 8
...
ggplot(df_convergencia, aes(x = n)) +
  ...
  labs(title = "Convergencia de la media muestral hacia la media teórica",
       subtitle = "Distribución unif(2,8) - Ley de los grandes números",
...
ggplot(data.frame(x = muestra_hist), aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)),  
                 fill = "lightblue", color = "black", alpha = 0.7, boundary = 2, binwidth = 0.25) +
  geom_hline(yintercept = 1/(b-a), color = "red", linewidth = 1.5, linetype = "dashed") +
  ...
  theme_minimal() +
  scale_x_continuous(breaks = seq(2, 8, by = 1), limits = c(1.5, 8.5)) +
  theme(
...
ggplot(data.frame(tiempo = tiempos_espera), aes(x = tiempo)) +
  geom_histogram(aes(y = after_stat(density)), 
                 fill = "orange", color = "black", alpha = 0.7, boundary = 0, binwidth = 2.5) +
  geom_vline(xintercept = tiempo_teorico, color = "red", linewidth = 1, linetype = "dashed") +
  geom_vline(xintercept = 10, color = "blue", linewidth = 1, linetype = "dotted") +
  labs(title = "Distribución de tiempo de espera en semáforo",
...
ggplot(data.frame(diametro = diametros), aes(x = diametro)) +
  geom_histogram(aes(y = after_stat(density)),  
                 fill = "lightgreen", color = "black", alpha = 0.7, boundary = 19.8, binwidth = 0.025) +
...
x_range <- seq(media_poblacional - 4*sqrt(varianza_poblacional/5), media_poblacional + 4*sqrt(varianza_poblacional/5), length.out = 200)
...
df_normal_teorica$n_factor <- factor(df_normal_teorica$n_factor, levels = levels(df_tcl$n_factor))
...
# Superponer curva normal teórica
  geom_line(data = df_normal_teorica, aes(x = x, y = y), 
            color = "red", linewidth = 1, alpha = 0.65) +
